  ...
  <ToolShell title="图片 OCR" description="识别图片中的文字（支持粘贴）">
    <div class="mx-auto grid w-full grid-cols-12">
      <div class="col-span-12 space-y-4 lg:col-span-8 lg:col-start-3">
        <!-- 无图片时：选择区域 -->
        <Panel v-if="!filePath" title="输入" hint="支持 Ctrl+V 粘贴">
          <template #actions>
        ...
        <template v-else>
          <Panel title="文件" :hint="fileName">
            ...
            <div class="flex items-center gap-3 text-xs">
              <ScanText class="size-4 shrink-0 text-muted-foreground" />
              <span v-if="loading" class="text-muted-foreground">识别中…</span>
              <span v-else-if="resultText" class="text-success">
                识别完成 · 置信度 {{ (confidence * 100).toFixed(1) }}%
              </span>
              <span v-else class="text-muted-foreground">等待识别</span>
            </div>
          </Panel>
